...
turtles-own[
  ...
  ;; -------------------------- Network
  in-link-count              ;; Count of ingoing directed links from other turtles
  out-link-count             ;; Count of outgoing directed links to other turtles

  ;; -------------------------- Model Parameter
  tax-morale                 ;; Float [0,1] (randomly assigned in initialise-population); showing the individuals motivation to pay taxes
...
to setup-network
  ...
  ask links[
    die]
  set out-link-count count out-link-neighbors
  set in-link-count count in-link-neighbors
...
to setup-homophilic-network
  ...
  ask individuals [
    if ((count out-link-neighbors + 1) < networksize)[
      ...
      let search (networksize - out-link-count - 1)
      if (search < 1) [stop]
...
to simulate-tax-morale
  ...
  ask n-of (updatingshare * num-individuals) turtles[
    ...
    let neighbors-tax-morale [tax-morale] of out-link-neighbors
    let neighbors-evading-morale [tax-morale] of out-link-neighbors with [tax-decision = 1]
    set social-tax-morale mean neighbors-tax-morale

    ;; Calculation of tax morale of the individual called by the function
    set tax-morale ((1 - socialweight) * tax-morale + socialweight * social-tax-morale)
    ...
    if (count out-link-neighbors with [tax-decision = 0] != 0)[
      set tax-paying-neighbors mean [tax-morale] of out-link-neighbors with [tax-decision = 0]]

    ;; Mean tax-morale of tax evading neighbors
    if (count out-link-neighbors with [tax-decision = 1] != 0)[
      set tax-evading-neighbors mean [tax-morale] of out-link-neighbors with [tax-decision = 1]]
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "if (count turtles > 0) [plot (count turtles with [tax-decision = 1]) / (count turtles)]"
"pen-1" 1.0 0 -2674135 true "" "if (count turtles > 0) [plot (count turtles with [tax-decision = 0]) / (count turtles)]"
...
578
Connected
count turtles with [out-link-count + 1 = networksize]
17
1
11
